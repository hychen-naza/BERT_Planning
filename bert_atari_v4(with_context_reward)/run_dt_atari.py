...
parser.add_argument('--data_dir_prefix', type=str, default='/home/hchen657/decision-transformer/original_dt/BERT_Planning/dt_atari/dqn_replay/')
...
class BERTDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #action_msk = np.concatenate([np.zeros((self.context,1)), np.ones((self.plan_horizon,1))], axis=0)
        action_msk = torch.from_numpy(action_msk).to(dtype=torch.bool)

        rtgs = torch.tensor(self.rtgs[context_start_idx:done_idx], dtype=torch.float32).unsqueeze(1)
        timesteps = torch.tensor(self.timesteps[context_start_idx:context_start_idx+1], dtype=torch.int64).unsqueeze(1)
        return states, actions, rtgs, action_msk, timesteps, init_state
...
bert_train_dataset = BERTDataset(obss, actions, done_idxs, rtgs, timesteps, rate, plan_horizon, context)
